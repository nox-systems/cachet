...
| [0021](0021-a-deployment-advertises-its-previous-public-keys.md) | A deployment advertises its previous public keys | A laptop set up after a rotation trusted only the new key and read every path signed before it as unverifiable; the config document now lists the earlier keys and `cachet setup` trusts them all |
